...
WORDS = set()
open('random_output.txt', 'w').close()  # clearing answer file
longest_word = 0
...
def find_sentences(text):
    ...
    while start > 0:
        rand = random.randint(0, len(beginning[start]) - 1)
        final_sentence.append(text[beginning[start][rand] + 1: start + 1])
        start = beginning[start][rand]

    final_sentence = list(reversed(final_sentence))
    final_sentence[0] = text[0] + final_sentence[0]

    # print(*final_sentence)
    f = open('random_output.txt', 'a')

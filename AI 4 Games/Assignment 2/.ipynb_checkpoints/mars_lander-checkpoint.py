...
#         self.population[:, :, 1] = np.round(self.population[:, :, 1]).astype(np.int)
    ...
    def mutation(self) -> None:
        ...
        # mutation for power
        random_indices = np.random.randint(0, self.d, size=(2, self.population_size))
    ...
    def run(self, iters: int, visualize_every=None) -> None:
        for i in range(iters):
            best_current_individual = self.population[self.cost.argmax()]
...
if __name__ == '__main__':
    ...
    model = ES(
        eval_f=lambda x: objective_function(
            ...
            l_start=l_start, r_start=r_start,
        ...
        population_size=50, chromosome_len=60, K=5
    )

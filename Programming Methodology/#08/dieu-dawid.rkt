...
(define (eval-env e env)
  (cond [(const? e)
        ...
        [(list?? e)
         (eval-env (list->cons (list-args e)) env )]
...
;;Dodalem w eval-env przypadek dla list i zdefiniowalem listy 
...
(define (list->cons e);; cukier dla list, zmiana listy na consy, zebysmy mogli ja evaluowac
  (if (null? e)
      'null
      (cons-cons (car e) (list->cons (cdr e)))))

(define (list-cons . l) ;; konstruktor listy
  (cons 'list l))

(define (list-args l) ;;selektor elementow z listy
  (cdr l))


(define eval-list-test (eval (list-cons 1 2 3 42 8 9)))
(define eval-list (eval (list-cons 1 2 3)))


;;

(define arith-eval
  '(lambda-rec (arith-eval expr)
               (cond [(pair? expr) (cond
                                     [(eq? (car expr) (quote +)) ;;jesli operator to '+, dodaje wyniki ewaluacji dwoch pozostalych wyrazen 
                                      (+ (arith-eval (car (cdr expr))) (arith-eval (car (cdr (cdr expr)))))]
                     ...
                     [true expr])));;nie mamy else, wiec true zawsze wykona sie w przeciwnym razie, zwracam wtedy wyrazenie, ktore nie jest juz w postaci binopa, czyli zwracam liczbe


;;TESTY 

(display "3 + 4 = ")
(eval (list arith-eval (list-cons
                          ''+ 3 4)))
